...
func (c *Client) offerNew(ctx context.Context) (*clientQUICConnection, error) {
	...
	// Begin Obfuscation: Send 5 to 10 random packets of 40 to 60 bytes
	for i := 0; i < 5+rand.Intn(6); i++ { // Randomly choose between 5 to 10 packets
		obfuscationPacket := make([]byte, 40+rand.Intn(21)) // Packet size between 40 to 60 bytes
		_, err := rand.Read(obfuscationPacket)              // Fill packet with random data
		if err != nil {
			udpConn.Close()
			return nil, E.Cause(err, "generating obfuscation packet")
		}
		_, err = udpConn.Write(obfuscationPacket) // Send the obfuscation packet
		if err != nil {
			udpConn.Close()
			return nil, E.Cause(err, "sending obfuscation packet")
		}
		time.Sleep(time.Millisecond * time.Duration(10+rand.Intn(11))) // Wait for 10 to 20 milliseconds
	}
	// End Obfuscation
